...
class HelpDeskService {
  async createHelpDesk(helpdeskData: Partial<IHelpDesk>, helpdeskImage: Express.Multer.File): Promise<IHelpDesk> {
    const imageName = await putObjectURL(helpdeskImage,"helpdesk");
    const helpdesk = await HelpDesk.create({title:helpdeskData.title,url:helpdeskData.url,image_name:imageName});
    return helpdesk;
  }
  ...
  async updateHelpDesk( helpdeskId: string, helpdeskData: IHelpDesk ,helpdeskImage:Express.Multer.File | undefined): Promise<IUpdatedHelpDesk | null> {
    ...
    const helpdesk = await HelpDesk.findByIdAndUpdate(helpdeskId,helpdeskData,{ new: true }
    );
    return helpdesk
  }
